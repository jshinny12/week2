...
describe('Custom Tests', function () {
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const aliceEthAddress = '0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC';
    const aliceChangeUtx = new Utxo({
      amount: aliceDepositAmount.sub(aliceWithdrawAmount).add(bobSendAmount),
      keypair: aliceDepositUtxo.keypair,
    })

    //const aliceDepositUtx = new Utxo({amount: aliceDepositAmount.sub(bobSendAmount), keypair: aliceDepositUtxo.keypair})

    let aliceDepositUtx
    try {
      aliceDepositUtx = Utxo.decrypt(aliceKeypair, events[0].args.encryptedOutput, events[0].args.index)
    } catch (e) {
      // we try to decrypt another output here because it shuffles outputs before sending to blockchain
      aliceDepositUtx = Utxo.decrypt(aliceKeypair, events[1].args.encryptedOutput, events[1].args.index)
    }

    await transaction({
      tornadoPool,
      inputs: [aliceDepositUtx],
      outputs: [aliceChangeUtx],
      recipient: aliceEthAddress,
      isL1Withdrawal: true,
    })
    ...
    expect(omniBridgeBalance).to.be.equal(0)
    const tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
    expect(tornadoPoolBalance).to.be.equal(utils.parseEther('0.12'))
